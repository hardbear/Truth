...
![Progress](http://progressed.io/bar/7?title=Difficulty:2400|Solved:<1000)
...
1. $$
   点与点之间形成若干线段，问题转化为求这些线段完全覆盖多少线段
   $$
...
3. $$
   读入被覆盖线段，r.pb(l)，表示存储以 r 为右端点的所有线段的左端点
   $$

4. $$
   读入询问的点，cur.pb(pre,i)，表示一条覆盖线段，同时记录询问编号
   $$

5. $$
   枚举点，枚举以这个点为右端点的覆盖线段，\\树状数组查询覆盖线段内的被覆盖线段的左端点
   $$

6. $$
   枚举以这个点为右端点的被覆盖线段的左端点，更新树状数组
   $$

7. $$
   显然第6步要在第5步之后做，\\使得当前更新的被覆盖线段的右端点一定小于等于覆盖线段的右端点
   $$

---

#### ✔ Codeforces 932E (组合数学 + dp)

**题意**

1. N 个人
2. 从 N 个人中选一个非空子集，子集中有 r 个人，则代价为 $r^k$
3. 求所有非空子集的代价之和
...
2. $$
   则，ans = \sum_{r=1}^N(C_N^r\cdot r^k)
   $$
